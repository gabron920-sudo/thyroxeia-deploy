...
app.post('/', async (c) => {
  ...
  try {
    ...
    if (action === 'create-subscription') {
      const { plan, noRefundsAcknowledged } = payload || {}
      if (!PLAN_PRICES[plan]) return c.json({ error: 'Invalid plan: ' + plan }, 400)
      if (noRefundsAcknowledged !== true) {
        return c.json({ error: 'You must acknowledge that subscription purchases are final and non-refundable before checkout.' }, 400)
      }
