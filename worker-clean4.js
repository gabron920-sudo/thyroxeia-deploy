...
import adminRouter from './routes/admin-clean.js'
...
app.get('*', async (c) => {
  ...
  // Avoid Cloudflare asset 500s on direct SPA routes by redirecting to the root app shell.
  // Client-side app state handles navigation after login.
  if (spaRoutes.has(url.pathname)) {
    return c.redirect('/?from=' + encodeURIComponent(url.pathname), 302)
  }
